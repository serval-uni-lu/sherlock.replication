...
public class CoverageTransformer implements ClassFileTransformer {
  ...
  public CoverageTransformer(final AgentConfigs agentConfigs) throws Exception {
    ...
    //Creating the list of packages by splitting the string withn ':'
    this.additionalPackages = Stream.of(agentConfigs.getAdditionalPackages().split(":")).collect(Collectors.toList());
  ...
  public byte[] transform(final ClassLoader loader, final String className,
    ...
    try {
      ...
      //Check wether the analysied class is contained in a the additonal packages to be instrumented
        for(String pakage:this.additionalPackages){
          if( pakage!="" &&cc.getPackageName().startsWith(pakage)){
            // logger.info(String.format("[SHERLOCK] %s should be instrumented !",cc.getName()));
